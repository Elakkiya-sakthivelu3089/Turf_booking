...
const EmployeeBookingLink = () => {
  ...
  const [bookedTimes, setBookedTimes] = useState(() => {
    const saved = sessionStorage.getItem("bookingEmployeeTimes");
  ...
  const loadBookings = useCallback(async () => {
    try {
      ...
      setSelectedSlot((currentSlot) => {
        ...
        const latestGame = data.games?.find((game) => game.id === selectedGame?.id);
        return latestGame?.slots.find((slot) => slot.startTime === currentSlot.startTime) || null;
      });
    } catch (err) {
  ...
  const handleJoinTeam = async (team) => {
    ...
    try {
      ...
      const bookedKey = `${selectedSlot.startTime}-${selectedSlot.endTime}`;
      setBookedTimes((current) => {
        const next = current.includes(bookedKey) ? current : [...current, bookedKey];
        sessionStorage.setItem("bookingEmployeeTimes", JSON.stringify(next));
        return next;
      });
      ...
      {linkClosed && (
        <section className="status-card employee-link-closed">
          ...
          <p className="muted-text">
            Employee booking is available only until the 7:00 PM slot closes at 8:00 PM.
          </p>
        </section>
      )}
      ...
      {employee && !linkClosed && (
        <div className="filter-bar today-slot-bar employee-session-bar">
          <div>
            ...
            <strong>{new Date(`${today}T00:00:00`).toLocaleDateString()}</strong>
          </div>
          <div>
          ...
          {closesAt && (
            <div>
              ...
              <strong>{new Date(closesAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</strong>
            </div>
          )}
        </div>
      )}
            ...
            <section className="booking-section">
              ...
              <div className="slot-grid">
                {selectedGame.slots.map((slot) => {
                  ...
                  const isBookedTime = bookedTimes.includes(slotKey);
                  ...
                  return (
                    ...
                    >
                      ...
                      {isBookedTime && !slot.isBreak && <small>Already booked</small>}
                    </button>
                  );
                })}
              </div>
